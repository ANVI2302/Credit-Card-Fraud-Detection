...
def preprocess_single(amount, hour, merchant, foreign, location, device, velocity, age):
    """Encode and scale a single transaction for prediction."""
    df = pd.DataFrame([{
        "amount":               amount,
...
def gauge_chart(prob, is_fraud):
    ...
    needle_angle = prob * 180   # 0° = left (legit), 180° = right (fraud)
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            "axis": {
                "range": [0, 100],
                "tickfont": {"color": "#64748b", "size": 11, "family": "Space Mono"},
                "ticksuffix": "%",
                "nticks": 6,
            },
        ...
        title={
            ...
            "font": {"size": 11, "color": "#64748b", "family": "Space Mono"},
        },
        domain={"x": [0, 1], "y": [0, 1]},
    ))
...
def batch_results_chart(df_results):
    ...
    fig.add_annotation(
        ...
        font=dict(color="#ef4444", family="Space Mono"),
        align="center",
    )
...
def confidence_bar_chart(probs):
    ...
    fig.add_hline(
        y=50, line_dash="dot", line_color="#f97316", line_width=1.5,
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#0d1117",
        plot_bgcolor="#111827",
        xaxis=dict(title="Transaction #", color="#64748b",
                   gridcolor="#1e2533", tickfont=dict(family="Space Mono", size=10)),
        yaxis=dict(title="Fraud Probability (%)", color="#64748b",
                   gridcolor="#1e2533", tickfont=dict(family="Space Mono", size=10),
                   range=[0, 105]),
        bargap=0.15,
    )
...
with tab_single:
    ...
    with col_form:
        ...
        with c2:
            hour = st.slider(
                "🕐 Transaction Hour", min_value=0, max_value=23,
                value=14, help="Hour of day (0–23)"
            )
            device = st.slider(
                "📱 Device Trust Score", min_value=0, max_value=100,
                value=72, help="Higher = more trusted device"
            )
            velocity = st.slider(
                "⚡ Velocity (last 24h)", min_value=0, max_value=20,
                value=2, help="Number of transactions in last 24 hours"
            )
            age = st.slider(
                "👤 Cardholder Age", min_value=18, max_value=90,
                value=35
            )
    ...
    with col_result:
        ...
        if analyze_btn:
            ...
            summary_items = [
                ...
                ("LOCATION",  "MISMATCH" if location else "OK"),
                ("DEVICE",    f"{device}/100"),
                ("VELOCITY",  f"{velocity} txns"),
                ("AGE",       f"{age} yrs"),
            ]
            for i, (label_txt, val) in enumerate(summary_items):
...
with tab_batch:
    ...
    with col_dl:
        ...
        st.download_button(
            ...
            file_name="fraud_check_template.csv",
            mime="text/csv",
    ...
    uploaded = st.file_uploader(
        "Upload transaction CSV for batch analysis",
        type=["csv"],
    ...
    if uploaded:
        try:
            ...
            # Remove already-processed columns for display
            for col in ["transaction_id", "is_fraud"]:
            ...
            def highlight_fraud(row):
                if row["Prediction"] == "FRAUD":
                    ...
                    return ["color: #86efac" if col == "Prediction"
                            else "color: #e2e8f0"
                            for col in row.index]
            ...
            <div style='font-family:Space Mono;font-size:0.78rem;color:#64748b;margin-top:0.5rem;'>
                Ensure your CSV contains all required columns with correct names and types.
            </div>
            """, unsafe_allow_html=True)
